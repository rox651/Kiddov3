...
import type { ProjectShowCase } from "../../domain/entities/project";
...
export default function ProjectsShowCase({ showcaseProjects, baseUrl }: ProjectsShowCaseProps) {
   ...
   const plane1Projects = showcaseProjects.slice(0, 3);
   const plane2Projects = showcaseProjects.slice(3, 6);
   const plane3Projects = showcaseProjects.slice(6, 8);

   return (
      <main className="relative w-full h-screen overflow-hidden" onMouseMove={manageMouseMove}>
         <div className="absolute z-10 right-20">
            <p className="text-white text-center max-w-3xl mx-auto text-6xl">
               Our work seamlessly blends creativity with conversions. We thrive on partnering with
               internal marketing, creative, and product teams to deliver impactful results.
               <br />
               <br />
               Explore our case studies to see how we bring strategic expertise and world-class
               creativity to life.
            </p>
         </div>
         <div ref={plane1} className="absolute inset-0 plane">
            {plane1Projects.map((project, idx) => (
               <img
                  ...
                  style={{ left: `${[90, 5, 35][idx] || 0}%`, top: `${[70, 65, 0][idx] || 0}%` }}
               />
            ))}
         </div>
         <div ref={plane2} className="absolute inset-0 plane">
            {plane2Projects.map((project, idx) => (
               <img
                  ...
                  style={{ left: `${[5, 80, 60][idx] || 0}%`, top: `${[10, 5, 60][idx] || 0}%` }}
               />
            ))}
         </div>
         <div ref={plane3} className="absolute inset-0 plane">
            {plane3Projects.map((project, idx) => (
               <img
                  ...
                  style={{ left: `${[65, 40][idx] || 0}%`, top: `${[2.5, 75][idx] || 0}%` }}
               />
            ))}
         </div>
      </main>
